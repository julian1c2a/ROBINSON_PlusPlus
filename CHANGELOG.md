...
- Pendiente: axiomas **no-ecuacionales** (ax18/19 orden vía `lt`/ax13/`∃`, ax21/24 mod2/`∨`), **listas** (ax_C3/ax_L3 — esquema de inducción sobre listas, distinto del de Nat), **Ax-P** (TFA) por inducción fuerte, y **Gödel Nivel D**. Empaquetado `⊢ axN` literal sale directo para `∀²` (ax6, ax10); para `∀³` (ax7, ax11, ax12) queda pendiente un helper n-ario (la derivación sustantiva `_ax` ya está).
