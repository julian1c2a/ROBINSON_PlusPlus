%% Las dos chapas de nivel. Se INCLUYE desde cap-los-lenguajes.tex (§2.5);
%% el encabezado lo pone el capítulo, no este fichero.
...
\noindent \textbf{Los puentes, y el que no existe.} Antes de la fórmula, qué es cada cosa, en una
línea: $\Prf_0\varphi$ dice que $\varphi$ tiene una demostración de Hilbert \emph{intuicionista} y
\emph{sin hipótesis}; $\Prf\varphi$, lo mismo pero admitiendo la lógica clásica; $\mathsf{PrfH}\
\Gamma\ \varphi$, lo mismo a partir de las hipótesis $\Gamma$; y
$\text{\ident{axioms}} \vdash \varphi$, que $\varphi$ se deriva de los axiomas en el cálculo
$\omega$. Las cuatro dicen «$\varphi$ es demostrable», y las cuatro dicen cosas distintas.
...
\]
\selee{si $\varphi$ se demuestra sin lógica clásica, se demuestra también con ella; si se demuestra
sin hipótesis, se demuestra bajo cualquier contexto $\Gamma$; y todo lo que se demuestra en el
cálculo finitario se deriva también en el cálculo $\omega$.}

\noindent Y el recíproco $\text{\ident{axioms}} \vdash \varphi \to \Prf\,\varphi$ \textbf{es falso, y
tiene que serlo}.
\selee{que algo se derive en el cálculo $\omega$ \emph{no} garantiza que tenga una demostración
finita.}
\noindent Si valiera, $\vdash$ sería r.e. y Tarski cerraría el paso. Ésa es la asimetría
que obliga a tener dos cálculos en vez de uno, y por eso la chapa distingue en cuál se afirma.
...
La cuarta fila del segundo eje —\emph{código}— es la que se cobró 31 módulos. \ident{tcFn} —«el código del
