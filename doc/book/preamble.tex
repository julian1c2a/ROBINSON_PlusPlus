...
% \defnot marca LA introducción de una notación; \selee da su lectura en voz
% alta. PLAN-LIBRO.md §2.8: una fórmula que se muestra se lee la primera vez.
\newcommand{\defnot}[1]{#1}
% Referencia a un capítulo o parte que todavía no está escrito. NO lleva
% número: un número escrito a mano deriva en cuanto se reordena el libro, y esa
% es la mentira silenciosa que todo el aparato de controles persigue. Cuando el
% capítulo exista, esto pasa a \cref sobre su \label.
\newcommand{\capfuturo}[1]{el capítulo dedicado a #1}
\newcommand{\partefutura}[1]{la parte dedicada a #1}

\newcommand{\selee}[1]{%
  \par\vspace{-0.3\baselineskip}
  {\small\itshape\color{rppMate}\hspace*{1.2em}se lee: #1\par}
...
% Un nombre cualificado (`FOL.MetaRules.imp_intro`) no se puede partir en \texttt
% y desborda; \fpi lo pasa por \nolinkurl, que parte por los puntos.
\newcommand{\fpi}[1]{\begingroup\urlstyle{tt}\nolinkurl{#1}\endgroup}
\newcommand{\footprint}[1]{%
  \par\vspace{-0.2\baselineskip}%
  {\raggedright\ttfamily\scriptsize\color{rppCom}\#print axioms $\rightarrow$ #1\par}}

% Footprint DECLARADO pero aún no medido con `make axiomas`. Distinguirlo del
% medido no es escrúpulo: §2.6 dice que una afirmación sin medir es testimonio.
\newcommand{\footprintpend}[1]{%
  \par\vspace{-0.2\baselineskip}%
  {\raggedright\ttfamily\scriptsize\color{rppCom}\#print axioms $\rightarrow$ #1
   \normalfont\sffamily\itshape (declarado, \textbf{sin medir})\par}}
...
% Código de OTRO proyecto, citado como evidencia. No está en el build de este
% libro y no sostiene ninguno de sus teoremas; por eso no lleva footprint y por
% eso la chapa dice de dónde viene.
\newcommand{\otroproyecto}[1]{%
  \par\vspace{-0.2\baselineskip}%
  {\sffamily\scriptsize\color{rppSondeo}%
   \fcolorbox{rppSondeo}{white}{\strut\,otro proyecto\,}\quad
   de \texttt{#1}: citado como evidencia, no es base de este libro\par}}
