/-
SONDEO (2026-09-05) — ¿es demostrable `∀t. hasWit (tcFn t)`?

Es el prerrequisito que §3.32.2 identificó para la vía C, y que la propagación de ADR-020
ha demostrado PORTANTE EN DOS SITIOS, no en uno:

1. el reflector Σ₁ del conjunto extra (`hasWitF` sobre código abstracto), y
2. el **sistema de prueba interno a nivel de código** — `pcc_exIntro_code'` (34 refs) y
   `pcc_forallElim_code` (18 refs) construyen líneas Q1/Q2 cuya carga es `tcFn a` con `a`
   ABSTRACTO, y bajo la enmienda esas líneas ya no son válidas sin la guarda.

Medido antes de empezar:
* `tcFn` es un ATOMO OPACO (`Term.func "tcFn" [t]`), caracterizado por SOLO DOS axiomas vivos:
  `ax_tc_zero` y `ax_tc_succ` (`ax_tc_cons` está RETIRADO por la reparación de inconsistencia).
* Luego para `t` abstracto no hay nada que desplegar ⟹ la única ruta es INDUCCION OBJETO
  sobre `zero`/`succ`, que es exactamente el fragmento que esos dos axiomas cubren.
* `prf_nat_induction` (`Meta/NatArithPrf.lean`) es la forma OBJETO del principio, y
  `NatArithPrf` es uno de los 9 imports que `CodeWitnessPrf` conserva tras la poda de ADR-020.
* Nadie lo ha intentado antes: cero apariciones de `hasWit (tcFn` en los 57 sondeos.

⚠️ LA MONEDA DE LA INDUCCION OBJETO (§3.31): el paso NO puede llevar la HI como hipótesis META.
Tiene que ser la implicación OBJETO `∀. Φ ⇒ Φ[σ#0]`, que es justo la firma de `prf_nat_induction`.
-/
...
/-! ## §11 · FOOTPRINT -/
#print axioms HasWitTcFn.prf_hasWit_tcFn
#print axioms HasWitTcFn.prf_argsIn_mono
#print axioms HasWitTcFn.prf_wfAll1_cons
