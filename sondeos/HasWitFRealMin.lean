/- # HW_transporte_min — LA MISMA prueba que `Probe/HW_transporte.lean`, pero sobre el
   FOOTPRINT MINIMO: solo la seccion `S_Clausura` (namespace `SinWTs`) y el bloque
   DEFINICIONAL `ENS` §1-§2 de `sondeos/EvalSubstfcPrf.lean`. 2090 lineas, ~10 s.

   Sirve para MEDIR que necesita la rama B (promocion a `Meta/`): la no-vacuidad NO
   depende de nada del descenso, ni de `Paso2`, ni de `SFsubsttc`, ni del ensamblaje.
-/
