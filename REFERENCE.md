...
| `Meta/Provability.lean` | `…Meta.Provability` | `Axioms`, `Meta.Godel`, `FOL.*` | ✅ Complete (Nivel C: `formCode`, `IsFormula`, `Provable` — **núcleo real**; capa legacy retirada en F7a) |
| ~~`Meta/Incompleteness.lean`~~ | — | — | ❌ **ELIMINADO (F7a, 2026-07-09)**: Gödel I/II legacy vía D2/D3 postulados → sustituido por la cadena real (`goedel_first_real'`/`goedel_second'`). Ver `AXIOMS.md` §3 |
...
| `Meta/CheckArith.lean` | `…Meta.CheckArith` | `Meta.StepArith` | ✅ Nivel D real F2.4: `validProofFn` (**19 step lemmas** `vpf_*`, incl. `vpf_thy`/`vpf_ind`) + `provFormulaC`/`provCodeC` Σ₁ |
| `Meta/Representability.lean` | `…Meta.Representability` | `Meta.{CheckArith,HilbertSeq,Induction}`, `Block6` | ✅ Nivel D real F2.5: `repr_pos : Prf φ → ⊢ provCodeC φ` (`proofCode`/`lineCode`/`vpf_run`) |
| `Meta/Necessitation.lean` | `…Meta.Necessitation` | `Meta.Representability` | ✅ Nivel D real F3: `d1`/`necessitation` + `goedel_first_unprovable_real` (Gödel I modular) |
| `Meta/Diagonal.lean` | `…Meta.Diagonal` | `Meta.{Necessitation,…}` | ✅ Nivel D real: `tcFn`/`tc_arith`/`diag_arith` + `godelC_fixedpoint` + **`goedel_first_real`** |
| `Meta/CodeDistinct.lean` | `…Meta.CodeDistinct` | `Meta.Provability`, `Block6` | ✅ Nivel D real F2.6: `formCode_ne` + familia (aritmética negativa de códigos) |
| `Meta/Induction.lean` | `…Meta.Induction` | `Meta.StepArith`, `Full.Induction` | ✅ Nivel D real F5: `ind_concl_code` (núcleo de la regla `ind` sólida → IΣ₁) |
| `Meta/ProofChain.lean` | `…Meta.ProofChain` | `Meta.CheckArith`, `Full.Lists` | ✅ Nivel D real R1–R4: verificador estructural `runFn`/`chainOk`/`lineOk`/`allIn` + compositividad/debilitamiento/monotonía + `provCodeC'` + validez de las 19 reglas (`lineWF`/`premsOf`) |
| `Meta/DerivCond.lean` | `…Meta.DerivCond` | `Meta.ProofChain` | ✅ Nivel D real: **D2** `⊢ provCodeC'(A⇒B) ⇒ (provCodeC' A ⇒ provCodeC' B)` |
| `Meta/Representability2.lean` | `…Meta.Representability2` | `Meta.{ProofChain,Representability}` | ✅ Nivel D real: encoder `proofCode'` + `runFn_track`/`chainOk_track` + **D1 = `repr_pos'`** (`#print axioms` = solo estándar) |
| `Meta/Reflection.lean` | `…Meta.Reflection` | `Meta.{DerivCond,Representability2}` | ✅ Nivel D real: combinadores internos `pcc_mp`/`pcc_andIntro`/`pcc_exIntro` (vía d2/repr_pos') |
| `Meta/DiagonalTwo.lean` | `…Meta.DiagonalTwo` | `Meta.{Representability2,Diagonal}` | ✅ Nivel D real: punto fijo `godelC'_fixedpoint` (⊢ G' ⇔ ¬provCodeC' G') + **`goedel_first_real'`** (Gödel I real estructural) |
| `Meta/GodelTwo.lean` | `…Meta.GodelTwo` | `Meta.{DerivCond,Reflection}` | 🔶 Gödel II núcleo: `con_imp_godel'`/`goedel_second'` (D2 real + **D3 postulado** `d3` + hipótesis punto-fijo/necesitación/⊬G) |
| `Meta/NumListPrf.lean` | `…Meta.NumListPrf` | `Meta.ReprPrf` | ✅ 12‑A fase 1a: ecuaciones `Prf` de `lenc`/`nthc` |
| `Meta/NatArithPrf.lean` | `…Meta.NatArithPrf` | `Meta.{ArithPrf,ChainPrf}` | ✅ 12‑A fase 1b: **toolkit aritmético de `<` en `Prf`** (inducción natural, `0+n=n`, `σm+n=σ(m+n)`, orden) |
| `Meta/BoundedInPrf.lean` | `…Meta.BoundedInPrf` | `Meta.{NumListPrf,NatArithPrf}` | ✅ 12‑A fase 1b: **`prf_In_iff_boundedIn`** (`In x L ⇔ ∃i<lenc L. nthc L i =eq x`) |
| `Meta/RunFnBoundedPrf.lean` | `…Meta.RunFnBoundedPrf` | `Meta.BoundedInPrf` | ✅ 12‑A fase 2 (lado `In`): `runFn nil` = *map* de `carc` → **`prf_In_runFn_iff`** |
| `Meta/ChainOkBoundedPrf.lean` | `…Meta.ChainOkBoundedPrf` | `Meta.RunFnBoundedPrf` | ✅ 12‑A fase 2 (lado `chainOk`): **`prf_chainOk_iff_chainOkB`** — el acumulador desaparece |
| `Meta/Sigma1BoundedPrf.lean` | `…Meta.Sigma1BoundedPrf` | `Meta.{ChainOkBoundedPrf,ReflectionPrf,Sigma1Prf}` | ✅ 12‑A fase 3 (puente): **`d3_prf_of_reflect_bounded`** — D3 reducida a reflejar la forma Δ₀ acotada |
| `Meta/Sigma1AtomPrf.lean` | `…Meta.Sigma1AtomPrf` | `Meta.{TrackedCorePrf,TcArithPrf}` | 🔄 12‑A fase 3 (núcleo): toolkit RASTREADO del átomo `=eq` (`eqCodeFn` + `prf_provCodeC'_eq_of_tracked`) |
| `Full/Induction.lean` | `…Full` | `Axioms`, `FOL.*` | ✅ ax6/7/10/11/12/18/19 derivados + `ax_induction`/`inductionFormula` |
...
> La tabla lista los módulos con sección propia en §3. El árbol real es mayor: **62 módulos fuente**
> (Minimal 11 + Meta 40 + Full 11) + el barrel `Meta.lean`. Build verde **76 jobs**, 0 errores,
> **0 warnings, 0 sorrys** (2026-07-09c, Lean v4.31.0). **7 declaraciones `axiom` de Lean** (tras F7a):
> 3 esquemas de inducción (`Full/`), TFA (`Block8`), 2 anclas de codificación, y `d3` (único
> postulado gödeliano vivo). Inventario autoritativo en **`AXIOMS.md`**. Ninguna es un `sorry` (ADR-008).

**Total histórico** (cifra congelada de 2026-06-12): 25 módulos fuente (Minimal 11 + Meta 3 + Full 11), build verde **39 jobs**, **0 sorrys, 0 warnings**. Las meta-reglas ω de deducción (`imp_intro`, `gen`, `raa`, `or_elim`, `ex_elim`) viven en `FOL/MetaRules.lean` (refactor 2026-06-12, re-export desde `Minimal.Axioms`); son `axiom`, no `:= sorry` (ADR-008). Meta-axiomas adicionales: `ax_p_tfa` (Block8), `ax_induction`/`ax_mod2_alternation`/`ax_list_induction` (Full), `Dem`/`diagonal_lemma`/`provFormula`/`provFormula_repr`/`dem_iff_provable` (Provability), `D2`/`D3` (Incompleteness).
...
**Estado 2026-07-08 — lo único vivo es D3.** Todo lo demás está cerrado:

1. **Gödel I REAL** ✅ sin postulados (`goedel_first_real'`, `Meta/DiagonalTwo.lean`).
2. **D1** ✅ (`repr_pos'_prf`) y **D2** ✅ (`d2_prf`, `#print axioms` = `[propext, choice, Quot.sound]`).
3. **D3** — reducida a la Σ₁‑completitud provable (`d3_prf_of_sigma1`). Se está construyendo por el
   **plan 12‑A** (§3.18‑§3.19): fases 1a/1b/2 ✅ (el verificador ya es Δ₀ y **sin acumulador**);
   **fase 3 en curso** (§3.19): ✅ puente `d3_prf_of_reflect_bounded` (D3 ⇐ reflejar la forma acotada)
   + ✅ átomo `=eq` rastreado; ⏳ reflexividad libre de muro, átomos `<`/`lineWF`, cuantificadores
   acotados, inducción estructural → `d3_prf` → `goedel_second_prf : ConsistentH → ¬ Prf Con'`.
   *Nota honesta:* 12‑A ≈ portar la Σ₁‑completitud provable de IΣ₁; es trabajo de varias sesiones,
   pero ya no queda ningún punto del plan sin verificar en código.
4. **Limpieza F7** — se parte en dos tras la auditoría de 2026-07-08 (`#print axioms` real):
   **F7a viable ya** (`goedel_second'` NO cita `diagonal_lemma`/`provFormula_repr`/`Dem`/
   `Incompleteness.D2`/`D3`; esos 7 postulados solo los usa la capa legacy → retirables, 14→7 `axiom`);
   **F7b bloqueada** (`GodelTwo.d3` sí es portante).
5. **Alternativa siempre disponible:** consolidar Gödel II **módulo el axioma D3** (`goedel_second'`).
   Estado ya publicable (Gödel I real + D1/D2 reales); D3 es notoriamente la pieza más dura de
   Gödel II también en Isabelle/Coq.

**Resumen histórico** (estado 2026-06-12):
