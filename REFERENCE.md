...
| 84k | **`PremsOfTagPrf`** | 🆕 🏁 **`premsOf` EVALUADO POR TAG — las 21 ramas, nivel OBJETO** (2026‑09‑10e, 417 l., **net‑0 PURO**). ⛔ **Levanta la obstrucción de §13.1/§3.59.2**: «los 21 `ax_premsOf_*` hacen pattern‑matching sobre un `cons` explícito ⇒ sobre una línea ABSTRACTA no hay nada que evaluar». **Cierto — mientras no se sepa la LONGITUD**, y la longitud sale del bicondicional del `ax_lineWF_*` del mismo tag. Cadena idéntica para los 21: tag ⇒ `lenc t ≐ n̄` ⇒ η (`prf_eta_lenc`) ⇒ la posición 1 **es** el tag (`prf_nthc1_carc_cdrc`) ⇒ `ax_premsOf_<k>` instanciado (**esquema**: un axioma por tag — `ax_premsOf_mp`, `ax_premsOf_gen`…). ⭐⭐ **Medido, y a la BAJA**: «21 casos» sugería 21 pruebas; son **dos lemas genéricos** (`prf_lenc_of_tag`, `prf_premsOf_of_tag`), **dos envoltorios** y **ocho líneas por tag**. Generalizar salió más barato porque lo caro era la instancia, no el esquema. ⚠️ **Hallazgo**: `ax_lineWF_mp` es el **único** cuyo bicondicional **no lleva condición estructural** (su fidelidad la liga entera `ax_premsOf_mp`) ⇒ `_tag_plain` frente a `_tag_and`. ➕ `prf_of_premsOf_branches`, el ensamblador. ⚠️ Esto es el nivel OBJETO; la reflexión **punteada** (B1) va encima. |
...
mecánicos (`botc` `implc` `andc` `orc` `exc`) y los dos duros. Los tres binarios salieron con **una
sola prueba**, porque `ax_substfc_impl/_and/_or` son la misma fórmula salvo el tag (certificado por
`rfl`).

⚠️ **Actualizado 2026‑09‑10h.** Este párrafo decía *«todo vive en `sondeos/` con el coste de
promoción sin pagar»* y citaba `pcc_eval_substtc'`/`pcc_eval_substtsc'` — los nombres **del sondeo**.
Ambas cosas están superadas: la promoción **se pagó** y en producción viven
**`pcc_eval_substtc`/`pcc_eval_substtsc`** (sin prima, `Meta/EvalSubsttcPrf.lean`), el ensamblaje
**está hecho** (`pcc_eval_substfc`, `Meta/EvalSubstfcPrf.lean`) y con él se cerró **C3** y **D3**.
