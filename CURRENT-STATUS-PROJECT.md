...
| Build status | ✅ Passing (**99 jobs**, 0 errores, **0 warnings**, 0 sorrys) |
| `NegVerifier` (módulo A) | ✅ **Decodificador COMPLETO** (§43): `Meta/CodeDecode.lean` (biyección de fórmulas: round‑trips + inyectividad `decodeForm_inj`) + `Meta/ChainDecode.lean` (cadenas: `decodeChain` + secciones `thy`/`mp`/`gen` + ensamblado `decodeChain_checkProof`/`decodeChain_prf`). Hallazgo: `lineJustif` es *lossy* ⟹ vale la **sección**, no el retract. Módulo B (21 tags, `Meta/LineWFCases.lean`): tabla + dirección negativa ✅; nivel `⊢` des‑duplicado (`Meta/LineWFDerives.lean`, `ProofChain` 870→540) ✅; **19 `ax_lineWF` estructurales reformulados a ACCESORES** ✅ (net‑0 axiomas; `prf_lineWF_<tag>` conservan enunciado; D1/D2 intactos) — `lineWF` ya reflejable sobre líneas abstractas. **Pendiente**: `pcc_lineWF_tracked` (B.3c, 21 casos de reflexión codificada) → `hC_dot`/D3 |
