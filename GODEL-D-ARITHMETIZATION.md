...
| **2.6** | representabilidad **negativa** `¬Dem d x → ⊢ᴴ ¬provCodeC φ` (reflexión/D3): la **solidez de `thy`** ya está (ver `thy-sound`); falta la **distinción de códigos** (probar `c ≠ ⌜axioma⌝`, `¬In c …`, numerales distintos) internalizada | ⬜ (parte `thy`-sound ✅) |
