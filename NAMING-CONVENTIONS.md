...
### Prefijo `ax_` para axiomas (verificado 2026-07-12)

A diferencia de FOL (proyecto hermano, donde los axiomas usan nombres descriptivos
planos sin prefijo), en ROBINSON_PlusPlus las declaraciones `axiom` llevan
consistentemente el prefijo `ax_` (`ax_induction`, `ax_list_induction`,
`ax_mod2_alternation`, `ax_inAxC`, `ax_p_tfa`, `ax_lineWF_cons`, …). Esto las hace
distinguibles a simple vista de los teoremas ordinarios sin necesidad de abrir el
fichero. Los cinco meta-axiomas de `Minimal/Axioms.lean` documentados en
