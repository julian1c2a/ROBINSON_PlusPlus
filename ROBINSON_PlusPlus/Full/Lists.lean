...
⚠️ **Genéricos en el contexto** (ADR‑023, 2026‑09‑10h): no citan ningún axioma —sólo `Derives.subst`/`Derives.refl`—, así que sirven **igual** sobre `axioms` y sobre `primAxioms`. `Meta/ProofChain.lean` los consume sobre `axioms`. -/

/-- `cons` respeta la igualdad en su segundo argumento. -/
theorem eq_congr_cons_right_full {Γ : List Formula} (x : Term) {a b : Term}
    (h : Γ ⊢ (a ≐ b)) : Γ ⊢ (cons x a ≐ cons x b) := by
...
theorem eq_congr_concat_left {Γ : List Formula} {u t₁ t₂ : Term} (h : Γ ⊢ (t₁ ≐ t₂)) :
    Γ ⊢ (concat u t₁ ≐ concat u t₂) := by
  ...
  have hS : ∀ s : Term, substFormula 0 s f = Formula.eq (concat u t₁) (concat u s) := by
    ...
    simp only [f, substFormula, concat, substTerm, substTerms,
               FOL.substTerm_liftTerm, if_true]
  exact (hS t₂) ▸ Derives.subst Γ t₁ t₂ f h ((hS t₁) ▸ Derives.refl Γ (concat u t₁))

/-- `concat` respeta la igualdad en su primer argumento. -/
theorem eq_congr_concat_right {Γ : List Formula} {t₁ t₂ : Term} (u : Term) (h : Γ ⊢ (t₁ ≐ t₂)) :
    Γ ⊢ (concat t₁ u ≐ concat t₂ u) := by
  ...
  have hS : ∀ s : Term, substFormula 0 s f = Formula.eq (concat t₁ u) (concat s u) := by
    ...
    simp only [f, substFormula, concat, substTerm, substTerms,
               FOL.substTerm_liftTerm, if_true]
  exact (hS t₂) ▸ Derives.subst Γ t₁ t₂ f h ((hS t₁) ▸ Derives.refl Γ (concat t₁ u))

/-- Substitución bajo `In`: de `t₁ ≐ t₂` y `In x t₁` deriva `In x t₂`. -/
theorem eq_subst_in {Γ : List Formula} {x t₁ t₂ : Term}
    (h_eq : Γ ⊢ (t₁ ≐ t₂)) (h_in : Γ ⊢ In x t₁) :
    Γ ⊢ In x t₂ := by
...
/-- `concat_assoc_pointwise_prim L M N : primAxioms ⊢ ((L##M)##N ≐ L##(M##N))`.

    Por inducción estructural sobre L (con M, N como parámetros Lean). -/
theorem concat_assoc_pointwise_prim (L M N : Term) :
    primAxioms ⊢ (concat (concat L M) N ≐ concat L (concat M N)) := by
  have base : primAxioms ⊢ (concat (concat nil M) N ≐ concat nil (concat M N)) := by
    -- ax_C1: nil ## L = L. So nil ## M = M y nil ## (M##N) = M##N.
    -- LHS: (nil ## M) ## N = M ## N. RHS: nil ## (M ## N) = M ## N. ✓
    have h_C1 := axp (by simp [primAxioms] : ax_C1_concat_nil ∈ primAxioms)
    have h_nil_M : primAxioms ⊢ (concat nil M ≐ M) := by
      have hh := spec h_C1 M
      simp [substFormula, substTerm, substTerms, concat, nil, zero] at hh
      exact hh
    have h_nil_MN : primAxioms ⊢ (concat nil (concat M N) ≐ concat M N) := by
      have hh := spec h_C1 (concat M N)
      simp [substFormula, substTerm, substTerms, concat, nil, zero] at hh
      exact hh
    have h_lhs : primAxioms ⊢ (concat (concat nil M) N ≐ concat M N) :=
  ...
  have step : ∀ h t : Term,
    ...
    have h_cht_M : primAxioms ⊢ (concat (cons h t) M ≐ cons h (concat t M)) := by
    ...
    have h_cht_MN : primAxioms ⊢ (concat (cons h t) (concat M N) ≐
      ...
      exact hh
    -- ax_C2 inst en (h, t##M, N): (cons h (t##M)) ## N = cons h ((t##M) ## N)
    have h_ctM_N : primAxioms ⊢ (concat (cons h (concat t M)) N ≐
...
/-- 🏁 **`ax_C3` CERTIFICADO sobre los primitivos** ([ADR‑023](../../DECISIONS.md), 2026‑09‑10h): el enunciado ya **no** es trivial por `ax`, porque `ax_C3_concat_assoc ∉ primAxioms`. -/
theorem concat_assoc_prim : primAxioms ⊢ ax_C3_concat_assoc := by
  ...
  exact concat_assoc_pointwise_prim L M N

/-! ### ax_L3 derivado: distribución de In sobre concat -/

/-- `in_concat_pointwise_prim x M L : primAxioms ⊢ (In(x, L##M) ⇔ In(x,L) ∨ In(x,M))`.

    Por inducción estructural sobre L (con x, M como parámetros Lean). -/
theorem in_concat_pointwise_prim (x M L : Term) :
    primAxioms ⊢ iff (In x (concat L M)) (lor (In x L) (In x M)) := by
  have base : primAxioms ⊢ iff (In x (concat nil M)) (lor (In x nil) (In x M)) := by
    -- nil ## M = M (ax_C1). In(x, nil) es false (ax_L1).
    -- ⇔: In x M ⇔ (In x nil ∨ In x M).
    have h_C1 := axp (by simp [primAxioms] : ax_C1_concat_nil ∈ primAxioms)
    have h_L1 := axp (by simp [primAxioms] : ax_L1_in_nil ∈ primAxioms)
    have h_nil_M : primAxioms ⊢ (concat nil M ≐ M) := by
      have hh := spec h_C1 M
      simp [substFormula, substTerm, substTerms, concat, nil, zero] at hh
      exact hh
    have h_M_nilM : primAxioms ⊢ (M ≐ concat nil M) := eq_symm h_nil_M
    have h_not_in_nil : primAxioms ⊢ neg (In x nil) := by
      have hh := spec h_L1 x
    ...
    · -- forward: In(x, nil##M) → In(x,nil) ∨ In(x,M)
      intro h_in_nilM
      have h_in_M : primAxioms ⊢ In x M := eq_subst_in h_nil_M h_in_nilM
  ...
  have step : ∀ h t : Term,
    ...
    have h_cht_M : primAxioms ⊢ (concat (cons h t) M ≐ cons h (concat t M)) := by
      ...
      simp [substFormula, substTerm, substTerms, concat, cons,
            FOL.substTerm_liftTerm, FOL.substTerm_liftLift] at hh
      exact hh
    have h_M_cht : primAxioms ⊢ (cons h (concat t M) ≐ concat (cons h t) M) :=
      eq_symm h_cht_M
    -- ax_L2 inst (x, h, t##M): In(x, cons h (t##M)) ⇔ (x=h ∨ In(x, t##M))
    have h_L2_tm : primAxioms ⊢ iff (In x (cons h (concat t M)))
    ...
    have h_L2_t : primAxioms ⊢ iff (In x (cons h t)) (lor (x ≐ h) (In x t)) := by
    ...
    · -- backward: In(x, cons h t) ∨ In(x, M) → In(x, cons h t ## M)
      ...
      · -- caso In(x, cons h t): aplicar h_L2_t forward → x=h ∨ In x t
        intro h_in_cht
        have h_disj_xhVin : primAxioms ⊢ lor (x ≐ h) (In x t) := iff_mp h_L2_t h_in_cht
        apply Minimal.Axioms.or_elim h_disj_xhVin
        · -- x = h: → In(x, cons h (t##M)) via h_L2_tm backward + or_intro_left
          intro h_xh
          have h_disj_xhVintm : primAxioms ⊢ lor (x ≐ h) (In x (concat t M)) :=
            Minimal.Axioms.or_intro_left h_xh
          have h_in_chtm : primAxioms ⊢ In x (cons h (concat t M)) :=
        ...
        · -- In x t: → In x (t##M) via IH backward (con or_intro_left), luego ax_L2 backward, luego eq_subst
          intro h_in_t
          have h_disj_inttM : primAxioms ⊢ lor (In x t) (In x M) :=
            Minimal.Axioms.or_intro_left h_in_t
          have h_in_tM : primAxioms ⊢ In x (concat t M) := iff_mpr IH h_disj_inttM
          have h_disj_xhVintm : primAxioms ⊢ lor (x ≐ h) (In x (concat t M)) :=
...
/-- 🏁 **`ax_L3` CERTIFICADO sobre los primitivos** ([ADR‑023](../../DECISIONS.md), 2026‑09‑10h). -/
theorem in_concat_prim : primAxioms ⊢ ax_L3_in_concat := by
  ...
  simp [substFormula, substTerm, substTerms, In, concat, lor, iff,
        FOL.substTerm_liftTerm, FOL.substTerm_liftLift]
  exact in_concat_pointwise_prim x M L

/-! ### 🏁 Y de vuelta a `axioms` por DEBILITAMIENTO — las firmas de siempre

`Derives.weakening` es **constructor**, así que esto no cuesta nada y **ningún consumidor cambia**.
La diferencia está arriba: lo que se demuestra es la versión **certificada**. -/

/-- **`ax_C3` como teorema en Full**: `⊢ ax_C3_concat_assoc`. -/
theorem concat_assoc_thm : axioms ⊢ ax_C3_concat_assoc := prim_to_axioms concat_assoc_prim

/-- **`ax_L3` como teorema en Full**: `⊢ ax_L3_in_concat`. -/
theorem in_concat_thm : axioms ⊢ ax_L3_in_concat := prim_to_axioms in_concat_prim

/-- Las versiones `axioms ⊢` de los dos *pointwise*, que es lo que consume
    `Meta/ProofChain.lean`. Por debilitamiento: gratis. -/
theorem concat_assoc_pointwise (L M N : Term) :
    axioms ⊢ (concat (concat L M) N ≐ concat L (concat M N)) :=
  prim_to_axioms (concat_assoc_pointwise_prim L M N)

theorem in_concat_pointwise (x M L : Term) :
    axioms ⊢ iff (In x (concat L M)) (lor (In x L) (In x M)) :=
  prim_to_axioms (in_concat_pointwise_prim x M L)
