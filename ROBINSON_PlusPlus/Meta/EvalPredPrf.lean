...
/-!
# `Meta/EvalPredPrf.lean` — la EVALUACIÓN DOTADA de `pred`

`pcc_eval_pred (n) : Prf (provFromCode (eqc (predcT (tcFn n)) (tcFn (pred n))))`, con `n`
**ABSTRACTO**: *el término simbólico «predecesor del numeral de `n`» es igual al numeral del valor
`pred n`*, demostrado **dentro de `Prov`**.

Es la pieza que pide `ax_substtc_var_gt` (`Minimal/Axioms.lean:447`), cuya cláusula devuelve
`varc (pred n)`.

**Molde**: `Meta/EvalArithPrf.lean` (`pcc_eval_add`) — base + paso + `prf_nat_induction` +
`prf_spec`. Resulta **más barato** que `add`: el paso inductivo **no usa la hipótesis de
inducción** (`ax26_pred_succ` da el valor directamente), así que el «paso» es incondicional.

⚠️ **`predcT` es una DEFINICIÓN y nada más**: no se postula ninguna ecuación de recursión suya.
...
open ROBINSON_PlusPlus.Meta.NatArithPrf ROBINSON_PlusPlus.Meta.CodeCtorKit
...
namespace ROBINSON_PlusPlus.Meta.EvalPredPrf
...
end ROBINSON_PlusPlus.Meta.EvalPredPrf

export ROBINSON_PlusPlus.Meta.EvalPredPrf (
  predcT predcT_termCode prf_congr_predcT prf_substtc_predcT prf_liftc_predcT
  substtc_inv_predcT evalPredCode pcc_eval_pred_zero pcc_eval_pred_succ
  prf_eval_pred_all pcc_eval_pred pcc_congr_predcT_code pcc_eval_varc_pred
)
