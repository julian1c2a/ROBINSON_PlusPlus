...
/-! ### Los 21 esquemas `lineWF`/`premsOf` a nivel `⊢` → `Meta/LineWFDerives.lean`

Estaban aquí, **probados por segunda vez desde los mismos axiomas** que sus gemelos `prf_*` de
`Meta/ReprPrf.lean`. Se movieron a `Meta/LineWFDerives.lean`, donde son el mero transporte por
`prf_to_derives`. Enunciados idénticos. -/
...
export ROBINSON_PlusPlus.Meta.ProofChain (
   runFn_nil runFn_cons congr_runFn_1 congr_runFn_2 concat_nil_eq concat_cons_eq runFn_concat in_cons_head in_cons_tail allIn_nil allIn_cons chainOk_nil chainOk_cons In_mono allIn_mono lineOk_mono concat_nil_right chainOk_subst1 chainOk_subst2 allIn_subst2 In_mono_right runFn_weaken chainOk_concat chainOk_mono provFormulaC' provCodeC' provCodeC'_intro provCodeC'_elim
)
