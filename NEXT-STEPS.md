...
3. ⏳ **VÍA GENUINA — Σ₁‑completitud provable estándar** (`substfc` + `num`, §11.3): D3 de libro. `substfc`/`substtc` **SÍ tiene ecuaciones de variable** (`ax_substtc_var_*`) — lo que falta a `tcFn` — y con ellas la inducción sobre `p` abstracto cierra. **PRIMER PASO (bloqueante, NO es código): decidir la codificación del testigo‑como‑número** (β‑función/secuencia) para que `runFn`/`chainOk`/`In` operen sobre él tras `num` (que aplana la lista). Luego: `num` object + ecuaciones → provable Δ₀‑completitud del verificador `⊢ ∀p (δ(φ,p) → Prov(⌜δ(φ,ṗ)⌝))` por inducción → ∃‑intro (`pcc_exIntro_code`) → `d3_prf` → `goedel_second_prf`.
