...
- ⛔ **SEGUNDA OBSTRUCCIÓN (§17): el `∃`‑intro de código exige testigo CERRADO.** `pcc_exIntro_code` pide `hw : ∀c, liftTerm c w = w`, pero tras el `∃`‑elim de `boundedIn` el testigo es la variable ligada `#0` y `tcFn #0` no es cerrado. **Es el viejo RIESGO‑1 reaparecido en la forma acotada** — corrige la afirmación previa de que `hbI` no estaba bloqueado: **ambas ramas pasan por aquí**. **NO es Tarski y es reparable** (§17.1): `hw` se usa en un solo punto (colapsar `liftTerm 0 (substfc zero w Ac)`); sin `w` cerrado no colapsa pero **es expresable** como `substfc zero (liftTerm 0 w) Ac` — hay que **arrastrar** el lift, no colapsarlo.
- ⏳ **SIGUIENTE BRICK (único, desbloquea las DOS ramas), §17.2:** **`pcc_exIntro_code'`** — generalización **lift‑aware** de `pcc_exIntro_code`, sin `hw`. Rehacer el ensamblaje `p ++ [q2line, mpline]` arrastrando `liftTerm 0 w`. Con él caen `hbI` (átomo `=eq` ✅ + `<`) y `hbC` (además `lineWF` ✅ vía `ax_lineWF_inv`) → `d3_prf_of_reflect_bounded` → `d3_prf` → `goedel_second_prf`.
