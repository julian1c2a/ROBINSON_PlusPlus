...
- ⛔ **OBSTRUCCIÓN ENCONTRADA (§16 del diseño): `lineWF`/`premsOf` NO están determinados.** Sus 21 axiomas son bicondicionales indexados por etiqueta de regla (`numeralM 0..20`) y **falta el axioma de inversión**; para una línea abstracta la teoría no puede extraer la etiqueta, luego `Prf (lineWF t ⇒ provCodeC' (lineWF t))` **no es derivable**. No afecta a la solidez de D1/D2/Gödel I (usan `lineWF` sólo en líneas concretas). **Arreglo mínimo propuesto (§16.4):** añadir `ax_lineWF_inv` a `Minimal/Axioms.lean` (un `def ax_*` de la teoría objeto, NO un `axiom` de Lean → los 7 de `AXIOMS.md` no cambian; precedente = `lenc`/`nthc` de fase 1a). **Requiere sanción explícita (amplía la teoría objeto).**
- ⏳ **Fase 3‑5 restante:** rama **`hbI` NO bloqueada** (sólo usa `=eq` cerrado + `<` reducible a `=eq` + `∃` acotado): reflexión de `<`, cuantificadores acotados (∀/∃), inducción estructural → `hbI`. Rama **`hbC` bloqueada** hasta resolver §16. Luego `d3_prf_of_reflect_bounded` → `d3_prf` → `goedel_second_prf`.
