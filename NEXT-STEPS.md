...
- ⏳ **Fase 3‑5 restante** (§15.5): (1) átomos `<`/`lineWF` (`atom1CodeFn`; `<` se reduce a `=eq` + `∃` acotado); (2) reflexión de cuantificadores acotados (∀/∃); (3) inducción estructural → `hbI`/`hbC` → `d3_prf` → `goedel_second_prf`.
