...
- 🔄 **Fase 3 núcleo — átomo `=eq` HECHO** (`Meta/Sigma1AtomPrf.lean`, §15.3 del diseño): toolkit RASTREADO del átomo de igualdad (`eqCodeFn` + congruencia + transporte + `prf_provCodeC'_eq_of_tracked`, espejo del de `In`). **Confirmado en código:** la reflexión de `=eq` para términos abstractos es imposible libre de muro (Tarski) → se rastrea con `tcFn` (que tiene congruencia) y el puente `tcFn t =eq termCode t` lo descarga la inducción de fase 5 (numerales, `prf_tc_numeral`).
- ⏳ **Fase 3‑5 restante** (§15.4): (1) reflexividad **libre de muro** (`provFromCode (eqCodeFn c c)` vía testigo del verificador para el axioma refl, sin tracking); (2) átomos `<`/`lineWF` (`atom1CodeFn`); (3) reflexión de cuantificadores acotados (∀/∃); (4) inducción estructural → `hbI`/`hbC` → `d3_prf` → `goedel_second_prf`.
